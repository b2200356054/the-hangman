...
for lap in words:
    ...
    while chances != 0:
        ...
        if " " not in theword.correctguessed:
            ...
            break
        if mod == 0:
            takeinput = str(input("Type a letter which you think the word --HAS-- or make a GUESS.\n "))
        elif mod == 1:
            takeinput = str(input("Type a letter which you think the word --HAS NOT-- or make a GUESS.\n"))
        
        if len(takeinput) == len(theword.word):
            check = [x for x in takeinput]    
            if check == theword.word:
